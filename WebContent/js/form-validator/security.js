	...
	$.formUtils
			.addValidator({
				...
				validatorFunction : function(value, $el, config, language,
						$form) {
					var conf = "", confInputName = $el.attr("name")
							+ "_confirmation", confInput = $form.find(
							'input[name="' + confInputName + '"]').eq(0);
					if (confInput) {
						conf = confInput.val()
					} else {
						console.warn('Could not find an input with name "'
								+ confInputName + '"')
					}
					return value === conf
				},
			...
			.addValidator({
				...
				validatorFunction : function(val, $el, conf) {
					...
					if (requiredStrength && requiredStrength > 3)
						requiredStrength = 3;
					return $.formUtils.validators.validate_strength
							.calculatePasswordStrength(val) >= requiredStrength
				},
				errorMessage : "",
				...
				calculatePasswordStrength : function(password) {
					...
					var checkRepetition = function(pLen, str) {
						...
						for ( var i = 0; i < str.length; i++) {
							var repeated = true;
							for ( var j = 0; j < pLen
									&& j + i + pLen < str.length; j++) {
								repeated = repeated
										&& str.charAt(j + i) == str.charAt(j
												+ i + pLen)
							}
							if (j < pLen) {
								repeated = false
				...
				strengthDisplay : function($el, options) {
					...
					$el
							.bind(
									...
									function() {
										var val = $(this).val();
										var $parent = typeof config.parent == "undefined" ? $(
												this).parent()
												: $(config.parent);
										var $displayContainer = $parent
												.find(".strength-meter");
										if ($displayContainer.length == 0) {
											$displayContainer = $("<span></span>");
											$displayContainer.addClass(
													"strength-meter").appendTo(
													$parent)
										}
										if (!val) {
										...
										var strength = $.formUtils.validators.validate_strength
	...
	$.formUtils
			.addValidator({
				...
				validatorFunction : function(val, $el, conf, lang, $form) {
					var backendValid = $el.valAttr("backend-valid"), backendInvalid = $el
							.valAttr("backend-invalid"), serverURL = document.location.href;
					if ($el.valAttr("url")) {
					...
					if ($.formUtils.isValidatingEntireForm) {
						$form.bind("submit", disableFormSubmit).addClass(
								"validating-server-side").addClass("on-blur");
						requestServer(
								serverURL,
								$el,
								val,
								...
								function() {
									$form.removeClass("validating-server-side")
											.removeClass("on-blur").get(0).onsubmit = function() {
									};
