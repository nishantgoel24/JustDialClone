/**
 * jQuery Form Validator ------------------------------------------ Created by
 * Victor Jonsson <http://www.victorjonsson.se>
 * 
...
(function($) {
	...
	$.fn.validateOnBlur = function(language, settings) {
		this.find("input[data-validation], textarea[data-validation]").blur(
				function() {
	...
	$.fn.showHelpOnFocus = function(attrName) {
		...
		this.find("textarea,input").each(
				function() {
					var $element = $(this), className = "jquery_form_help_"
							+ $element.attr("name"), help = $element
							.attr(attrName);
					if (help) {
						$element
								.focus(
										function() {
											var $help = $element.parent().find(
													"." + className);
											if ($help.length == 0) {
												$help = $("<span />").addClass(
														className).addClass(
	...
	$.fn.validateInputOnBlur = function(language, config, attachKeyupEvent,
		...
		var $element = this, elementErrMsgObj = document
				.getElementById($element.attr("name") + "_err_msg"), $form = $element
				.closest("form"), validationRule = $element
				.attr(config.validationRuleAttribute);
		$element.removeClass(config.errorElementClass).css("border-color", "")
				.parent().find("." + config.errorMessageClass).remove();
		...
		if (validation === true) {
			...
			$element.removeClass("valid").parent().removeClass("has-error")
					.removeClass("has-success")
		} else {
			$element.addClass(config.errorElementClass).removeClass("valid")
					.parent().addClass("has-error").removeClass("has-success");
			if (elementErrMsgObj != null) {
				elementErrMsgObj.innerHTML = validation
			} else {
				var $parent = $element.parent();
				$parent.append('<span class="' + config.errorMessageClass
						+ ' help-block">' + validation + "</span>");
	...
	$.fn.validateForm = function(language, config) {
		...
		var addErrorMessage = function(mess, $element) {
			if (mess !== null) {
				...
				$element.valAttr("current-error", mess).removeClass("valid")
						.parent().removeClass("has-success")
			}
		}, errorMessages = [], errorInputs = [], $form = this, ignoreInput = function(
				name, type) {
		...
		$form.find("input,textarea,select").filter(
				':not([type="submit"],[type="button"])').each(
				function() {
					var $element = $(this);
					var elementType = $element.attr("type");
					if (!ignoreInput($element.attr("name"), elementType)) {
						var validation = $.formUtils.validateInput($element,
								language, config, $form, "submit");
						if (validation !== true) {
							addErrorMessage(validation, $element)
						} else {
							$element.valAttr("current-error", false).addClass(
									"valid").parent().addClass("has-success")
						}
					}
				});
		$form.find(".has-error").removeClass("has-error");
		...
		if (!$.formUtils.haltValidation && errorInputs.length > 0) {
			...
			$.each(errorInputs, function(i, $input) {
				...
				$input.addClass(config.errorElementClass).parent().addClass(
						"has-error")
			});
			if (config.errorMessagePosition === "top") {
				...
				$form.children().eq(0)
						.before(
								'<div class="' + config.errorMessageClass
				...
				$.each(errorInputs, function(i, $input) {
					var $parent = $input.parent(), $errorSpan = $parent
							.find("span[class=" + config.errorMessageClass
									+ "]");
					if ($errorSpan.length > 0) {
						$errorSpan.text(", " + $input.valAttr("current-error"))
					} else {
						$parent.append('<span class="'
								+ config.errorMessageClass + ' help-block">'
								+ $input.valAttr("current-error") + "</span>")
					}
				})
			}
			return false
	...
	$.validationSetup = function(config) {
		if (typeof console != "undefined" && console.warn) {
			window.console
					.warn("Using deprecated function $.validationSetup, pls use $.validate instead")
		}
		$.validate(config)
	};
	$.formUtils = {
		...
		loadModules : function(modules, path, fireEvent) {
			...
			var loadModuleScripts = function(modules, path) {
				...
				var cacheSuffix = "?__=" + (new Date).getTime(), appendToElement = document
				...
				var findScriptPathAndLoadModules = function() {
					...
					$("script")
							.each(
									function() {
										if (this.src) {
											var scriptName = this.src
													.substr(
															this.src
																	.lastIndexOf("/") + 1,
															this.src.length);
											if (scriptName
													.indexOf("jquery.form-validator.js") > -1
													|| scriptName
															.indexOf("jquery.form-validator.min.js") > -1) {
												foundPath = this.src
														.substr(
																0,
																this.src
		...
		validateInput : function($element, language, config, $form,
				eventContext) {
			var value = $.trim($element.val() || ""), optional = $element
					.valAttr("optional"), validationDependsOnCheckedInput = false, validationDependentInputIsChecked = false, validateIfCheckedElement = false, validateIfCheckedElementName = $element
					.valAttr("if-checked");
			...
			if (!value && optional === "true"
					|| validationDependsOnCheckedInput
					&& !validationDependentInputIsChecked) {
			...
			$
					.split(
							...
							function(rule) {
								...
								if (validator
										&& typeof validator["validatorFunction"] == "function") {
									...
									if (eventContext != "keyup"
											|| validator.validateOnKeyUp) {
										isValid = validator.validatorFunction(
												value, $element, config,
												language, $form)
									}
									if (!isValid) {
										validationErrorMsg = $element
												.attr(config.validationErrorMsgAttribute);
									...
									console.warn('Using undefined validator "'
											+ rule + '"')
								}
							}, " ");
		...
		parseDate : function(val, dateFormat) {
			var divider = dateFormat.replace(/[a-zA-Z]/gi, "").substring(0, 1), regexp = "^", formatParts = dateFormat
					.split(divider), matches, day, month, year;
		...
		lengthRestriction : function($inputElement, $maxLengthElement) {
			...
			$($inputElement).bind("keydown keyup keypress focus blur",
					countCharacters).bind("cut paste", function() {
		...
		numericRangeCheck : function(value, rangeAllowed) {
			...
			if (range.length == 2
					&& (value < parseInt(range[0], 10) || value > parseInt(
							range[1], 10))) {
		...
		suggest : function($element, suggestions, settings) {
			...
			$element
					...
					.bind(
							"keyup",
							function() {
								var $input = $(this), foundSuggestions = [], val = $
										.trim($input.val()).toLocaleLowerCase();
								if (val == $.formUtils._previousTypedVal) {
									return
								} else {
									$.formUtils._previousTypedVal = val
								}
								var hasTypedSuggestion = false, suggestionId = $input
										.valAttr("suggestion-nr"), $suggestionContainer = $(".jquery-form-suggestion-"
										+ suggestionId);
								...
								if (hasTypedSuggestion
									...
									$suggestionContainer = $("<div></div>")
											.css(config.css).appendTo("body");
									$suggestionContainer
											.addClass("jquery-form-suggestions");
									$suggestionContainer
											.addClass("jquery-form-suggestion-"
													+ suggestionId)
								} else if (foundSuggestions.length > 0
							...
							function(e) {
								...
								if (code == 13
									...
									if ($suggestions.length > 0
										...
										var containerInnerHeight = $suggestionContainer
												.innerHeight(), containerScrollTop = $suggestionContainer
												.scrollTop(), suggestionHeight = $suggestionContainer
										...
										if (activeSuggestionPosY < containerScrollTop
											...
											$suggestionContainer
													.scrollTop(activeSuggestionPosY)
										}
										$suggestions
												.removeClass(
														"active-suggestion")
		...
		LANG : {
			...
			requiredFields : "Answer all the required fields",
			badTime : "You have not given a correct time",
			badEmail : "Enter the correct e-mail address",
			badTelephone : "Enter the correct phone number",
			...
			badStrength : "  Minimum 8 Characters ",
	...
	$.formUtils.addValidator({
		...
		validatorFunction : function(email) {
			...
			if (emailParts.length == 2) {
				return $.formUtils.validators.validate_domain
						.validatorFunction(emailParts[1])
			...
			.addValidator({
				...
				validatorFunction : function(val, $input) {
					var topDomains = [ ".com", ".net", ".org", ".biz", ".coop",
							...
							".rw", ".ru", ".sa", ".sb", ".sc", ".sd", ".se",
							".sg", ".sh", ".si", ".sj", ".sk", ".sl", ".sm",
							".sn", ".so", ".sr", ".st", ".sv", ".sy", ".sz",
							".tc", ".td", ".tf", ".tg", ".th", ".tj", ".tk",
							".tm", ".tn", ".to", ".tp", ".tr", ".tt", ".tv",
							".tw", ".tz", ".ua", ".ug", ".uk", ".um", ".us",
							".uy", ".uz", ".va", ".vc", ".ve", ".vg", ".vi",
							".vn", ".vu", ".ws", ".wf", ".ye", ".yt", ".za",
							".zm", ".zw", ".mobi", ".xxx", ".asia" ], ukTopDomains = [
							"co", "me", "ac", "gov", "judiciary", "ltd", "mod",
							"net", "nhs", "nic", "org", "parliament", "plc",
							"police", "sch", "bl", "british-library", "jet",
							"nls" ], dot = val.lastIndexOf("."), domain = val
							.substring(0, dot), ext = val.substring(dot,
							val.length), hasTopDomain = false;
					...
					if (!hasTopDomain) {
						...
						var lastChar = domain.substring(domain.length - 1,
								domain.length);
	...
	$.formUtils
			.addValidator({
				...
				validatorFunction : function(val, $el, config, lang) {
					var lengthAllowed = $el.valAttr("length"), type = $el
							.attr("type");
					...
					var len = type == "file" && $el.get(0).files !== undefined ? $el
							.get(0).files.length
							: val.length, lengthCheckResults = $.formUtils
							.numericRangeCheck(len, lengthAllowed), checkResult;
	...
	$.formUtils
			.addValidator({
				...
				validatorFunction : function(val, $el, config) {
					if (val !== "") {
						var allowing = $el.valAttr("allowing") || "", decimalSeparator = $el
								.valAttr("decimal-separator")
								|| config.decimalSeparator;
						if (allowing.indexOf("number") == -1)
							allowing += ",number";
						if (allowing.indexOf("negative") > -1
								&& val.indexOf("-") === 0) {
							val = val.substr(1)
						}
						if (allowing.indexOf("number") > -1
						...
						if (allowing.indexOf("float") > -1
								&& val.match(new RegExp("^([0-9]+)\\"
										+ decimalSeparator + "([0-9]+)$")) !== null) {
							return true
						}
					}
					return false
	...
	$.formUtils
			.addValidator({
				...
				validatorFunction : function(val, $el, config, language) {
					var patternStart = "^([a-zA-Z0-9", patternEnd = "]+)$", additionalChars = $el
	...
	$.formUtils.addValidator({
		...
		validatorFunction : function(val, $el, config, lang, $form) {
			...
			var checkedCount = $("input[type=checkbox][name^='" + elname
					+ "']:checked", $form).length;
